...
/// Displays the followers list for a given user.
///
/// When [userId] is empty the screen loads the current user's own followers
/// via `GET /social/followers`; otherwise it loads `GET /social/users/{id}/followers`.
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<UserSummaryPage>(
      ...
      builder: (context, snapshot) {
        ...
        if (items.isEmpty) {
          return const Center(child: Text('아직 팔로워가 없어요'));
        }
...
class _FollowChipState extends ConsumerState<_FollowChip> {
  // Local optimistic state — avoids a full list reload after each toggle.
