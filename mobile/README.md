...
Book Club의 시각 언어는 **Claude (Anthropic) 디자인 시스템**을 Flutter 로 이식한 것이다. 설계 원문은 `/awesome-design-md/design-md/claude/DESIGN.md` 를 참조한다.
...
- **등급별 컬러 테마**: `lib/core/theme/grade_theme.dart`
  - `ReaderGrade` 5단계(새싹 독자 → 서재 마스터)에 각각 대응하는 `ColorScheme` 을 `GradeTheme.schemes` / `GradeTheme.darkSchemes` 에서 제공한다.
...
- **커스텀 토큰 (ThemeExtension)**
  - `AppSpacing` — `xs / sm / md / lg / xl` 간격 스케일
  - `AppRadius` — `sm / md / lg / pill` 코너 반경
  - `AppShadows` — Claude 의 ring 기반 halo 와 whisper 드롭섀도우
  - 접근 패턴:
    ```dart
