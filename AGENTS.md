# AGENTS.md — Book Club 프로젝트 규칙 (에이전트용)
...
### 0.1 티켓 작성 양식 (강제 · 2026-08-22 이후 생성 티켓)

**양식을 쓰는 티켓**: 스토리(부모), 에픽, 그리고 부모 없이 단독으로 올리는 티켓(버그·백로그 건).
**양식을 쓰지 않는 티켓**: 하위 태스크 — 기한만 남긴다.

기존 티켓은 소급하지 않는다.

**제목 말머리**

- `[기획/디자인 필요]` — 기획·디자인이 선행되어야 착수할 수 있는 건
- 말머리 없음 — 바로 개발 가능한 건
- 하위 태스크는 작업 영역 접두사를 쓴다: `[BE]` / `[FE]` / `[환경]` / `[리서치]`

**본문 (맨 위부터 이 순서)**

```
[왜 하는가]
이 작업이 필요해진 배경. 지금 무슨 일이 벌어지고 있나

[무엇을 하는가]
바뀌는 화면이나 동작

[고객 영향]
안 고치면 고객이 뭘 못 하나, 고치면 고객이 뭘 할 수 있나 (내부 작업이면 "내부 전용")

[등록 계기]
어디서 나온 건인지 (리더 문의, CS 접수 3건, 내부 점검 중 발견, 데이터 확인 중 발견 등)
...
- 날짜는 본문에 쓰지 않고 Jira `기한` 필드에 넣는다.
- **에픽·스토리의 기한 = 실 배포일** (서비스 반영 일자)
- **하위 태스크의 기한 = 그 작업 완료 예정일**
- 등록일·착수일은 적지 않는다. Jira 생성일과 티켓 상태값으로 판단한다.

`기한` 의 뜻은 위로 통일한다. 4~5월 티켓에는 작업 마감일 뜻으로 들어가 있어 혼동을 막기 위해 적어둔다.
